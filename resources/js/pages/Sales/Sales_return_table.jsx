...
export default function SalesReturnList() {
  ...
  useEffect(() => {
    document.title = "Sale Return | Dashboard";
    fetchReturns();
  }, []);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Sale Returns"); 

    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });

    saveAs(
      new Blob([excelBuffer], { type: "application/octet-stream" }),
      `Sale_Returns_${new Date().toISOString().split("T")[0]}.xlsx` 
    );
  };
  ...
  return (
    <div className="container-fluid px-md-10">
      ...
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 p-4 rounded-4 shadow-sm"
        ...
        <div>
          <h3 className="fw-bold mb-1 text-white">🧾 Sale Returns</h3>
          <p className="mb-0 text-white-50">Manage and track returned inventory</p>
        </div>
        <div className="d-flex gap-2 mt-3 mt-md-0">
          <Link to="/sale-return-form" className="btn btn-light fw-semibold px-4 py-2" style={{ borderRadius: "10px" }}>
            <FaPlus className="me-2" /> Return Sale
          </Link>
